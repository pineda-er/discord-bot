...
#---------------------------------------------------------------------
            ...
            try:
                ...
                if not archive_doc:
                    ...
                    try:
                        for t in archive_channel.threads:
                            if username.lower() in t.name.lower():
                                thread = t
                                thread.edit(name=f"{username}") 
                                break
                        if not thread:
                ...
                try:
                    ...
                    embed = create_embed(description=f"{SEARCH_ICON} Fetching new videos found for `{username}`", colour=0x6ac5fe)
                    await interaction.edit_original_response(embed=embed)
                    videos = await utils_fetch_video_info(sec_uid, 105, create_time)
                    print(videos)
                except Exception as e:
                    ...
                    return
                if not videos:
                    print(f"[archive] No new videos found for {username}")
                ...
                try:
                    ...
                    for video in videos:
                        try:
                            ...
                            await thread.send(content=f"[[Watch on Tiktok]](<{tiktok_url}>)", file=file)
                        except Exception as e:
                    ...
                    await interaction.edit_original_response(embed=embed)
                    return
                embed = create_embed(description=f"{SUCCESS_ICON} **Fetched {len(videos) if videos else 0} videos.**\n\nArchive thread: [Go to topic]({thread.jump_url})", colour=0x77dd77)
                await interaction.edit_original_response(embed=embed)
            except Exception as e:
    ...
    def send_embed_safe(self, interaction, embed):
        try:
            if interaction.response.is_done():
                return interaction.followup.send(embed=embed, ephemeral=False)
            else:
                return interaction.response.send_message(embed=embed, ephemeral=False)
        except Exception as e:
